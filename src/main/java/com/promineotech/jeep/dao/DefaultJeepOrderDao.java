...
public class DefaultJeepOrderDao implements JeepOrderDao {
  ...
  public Optional<Jeep> fetchModel(JeepModel model, String trim, int doors) {
    String sql =
        "SELECT * FROM models WHERE model_id = :model_id AND trim_level = :trim_level AND num_doors = :num_doors";
  ...
  class CustomerResultSetExtractor implements ResultSetExtractor<Customer> {
    ...
    public Customer extractData(ResultSet rs) throws SQLException, DataAccessException {
      ...
      // formatter:off
      return Customer.builder().customerId(rs.getString("customer_id"))
          .customerPK(rs.getLong("customer_pk")).firstName(rs.getString("first_name"))
          .lastName(rs.getString("last_name")).phone(rs.getString("phone")).build();
  ...
  class JeepResultSetExtractor implements ResultSetExtractor<Jeep> {
    ...
    public Jeep extractData(ResultSet rs) throws SQLException, DataAccessException {
      ...
      // formatter:off
      return Jeep.builder().modelId(JeepModel.valueOf(rs.getString("model_id")))
          .modelPK(rs.getLong("model_pk")).trimLevel(rs.getString("trim_level"))
          .numDoors(rs.getInt("num_doors")).wheelSize(rs.getInt("wheel_size"))
          .basePrice(rs.getBigDecimal("base_price")).build();
  ...
  class ColorResultSetExtractor implements ResultSetExtractor<Color> {
    ...
    public Color extractData(ResultSet rs) throws SQLException, DataAccessException {
      ...
      // formatter:off
      return Color.builder().colorId(rs.getString("color_id")).colorId(rs.getString("color_id"))
          .colorPK(rs.getLong("color_pk")).isExterior(rs.getBoolean("is_exterior"))
  ...
  class TireResultSetExtractor implements ResultSetExtractor<Tire> {
    ...
    public Tire extractData(ResultSet rs) throws SQLException, DataAccessException {
      ...
      // formatter:off
      return Tire.builder().manufacturer(rs.getString("manufacturer"))
          .price(rs.getBigDecimal("price")).tireId(rs.getString("tire_id"))
          .tirePK(rs.getLong("tire_pk")).tireSize(rs.getString("tire_size"))
          .warrantyMiles(rs.getInt("warranty_miles")).build();
  ...
  class EngineResultSetExtractor implements ResultSetExtractor<Engine> {
    ...
    public Engine extractData(ResultSet rs) throws SQLException, DataAccessException {
      ...
      // formatter:off
      return Engine.builder().description(rs.getString("description"))
          .engineId(rs.getString("engine_id")).enginePK(rs.getLong("engine_pk"))
          .fuelType(FuelType.valueOf(rs.getString("fuel_type")))
          .hasStartStop(rs.getBoolean("has_start_stop")).mpgCity(rs.getFloat("mpg_city"))
          .mpgHwy(rs.getFloat("mpg_hwy")).name(rs.getString("name"))
          .price(rs.getBigDecimal("price")).sizeInLiters(rs.getFloat("size_in_liters")).build();
  ...
  public Order saveOrder(Customer customer, Jeep jeep, Color color, Engine engine, Tire tire,
    ...
    Order order = Order.builder().customer(customer).model(jeep).color(color).engine(engine)
        .orderPK(orderPK).tire(tire).options(options).price(price).build();
    return order;
  }
  ...
  private SqlParams generateInsertSql(Option option, Long orderPK) {
    ...
    String sql = "INSERT INTO order_options (" + "option_fk, order_fk" + ") VALUES ("
        + ":option_fk, :order_fk" + ")";
